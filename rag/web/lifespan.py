...
def create_dependency_container():
    llm = OpenAI(model="gpt-3.5-turbo", temperature=0, api_key=settings.openai_key)
    embeder = OpenAIEmbedding(api_key=settings.openai_key)

    di[BaseImagePreprocess] = StandardImagePreprocess()
    di[BaseOpticalCharacterRecognizer] = EasyOCRProvider()
